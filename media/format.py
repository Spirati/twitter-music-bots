...
generate_ffmpeg_string = lambda filename,cover,album: f'{os.path.join(ROOT_DIR, "ffmpeg")} -nostdin -y -loglevel error -loop 1 -i "input/{cover}" -i "input/{filename}" -filter:v "fps=30,scale=w=480:h=480" -tune stillimage -c:a aac -c:v libx264 -b:a 192k -pix_fmt yuv420p -t 140 -shortest "out/{album} - {filename}-process.mp4"'
...
def process_files():
    ...
    files = listdir(os.path.join(ROOT_DIR, "input"))
    cover = None
    songs = []
    ...
    try:
        mkdir(os.path.join(ROOT_DIR, "out"))
    except:
        rmtree(os.path.join(ROOT_DIR, "out"))
        mkdir(os.path.join(ROOT_DIR, "out"))
